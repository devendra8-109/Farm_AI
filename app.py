...
if "y_state"          not in st.session_state: st.session_state.y_state          = "madhya pradesh"  # sensible default
if "y_district"       not in st.session_state: st.session_state.y_district       = "All Districts"
if "y_crop"           not in st.session_state: st.session_state.y_crop           = "wheat"           # sensible default
if "_last_autofill_state" not in st.session_state: st.session_state._last_autofill_state = ""
if "app_initialized"  not in st.session_state: st.session_state.app_initialized  = True  # skip onboarding on restarts
...
# ONE-QUESTION LANDING SCREEN — only shown when explicitly navigated to
# ─────────────────────────────────────────────────────────────────────────────
...
if page == "Overview":
    ...
    # ── 4 KPI CARDS (with full error protection) ──────────────────────────────
    try:
        ...
        if not _crop_prof_ai.empty:
            ...
            _px_rows = df_price[df_price['crop'] == _res_c_h] if not df_price.empty and _res_c_h else pd.DataFrame()
            _cp_h    = float(_px_rows['avg_modal_price'].mean()) if not _px_rows.empty else 2000.0
    ...
    try:
        ...
        _mtp_h          = (_months_order_h.index(_peak_home) - _months_order_h.index(_curr_m_h)) % 12
        _sell_label     = "Sell Now! ✅" if _mtp_h == 0 else f"Sell in {_mtp_h}m ⏳"
        _sell_color     = "#15803d" if _mtp_h == 0 else "#f59e0b"
    except Exception:
        _peak_home, _sell_label, _sell_color = "N/A", "Check Calendar", "#64748b"

    try:
        _res_c_h  = resolve_price_crop(y_crop, price_crops_monthly)
        _px2      = df_price[df_price['crop'] == _res_c_h] if not df_price.empty and _res_c_h else pd.DataFrame()
        _price_h  = int(_px2['avg_modal_price'].mean()) if not _px2.empty else 0
    except Exception:
        _price_h = 0
    ...
    <div style="display:grid; grid-template-columns:repeat(4,1fr); gap:12px; margin-bottom:20px;">
        <div style="background:#f0fdf4; border:2px solid #bbf7d0; border-radius:16px; padding:18px; text-align:center;">
            <div style="font-size:11px; color:#64748b; font-weight:700; text-transform:uppercase; letter-spacing:1px;">📦 Predicted Yield</div>
            <div style="font-size:26px; font-weight:800; color:#0f172a; margin:8px 0 4px;">{_yield_h} <span style="font-size:13px;font-weight:500;color:#64748b;">t/ha</span></div>
            <div style="font-size:12px; color:#15803d; font-weight:600;">{y_crop.title()} · {y_state.title()}</div>
        </div>
        <div style="background:#fffbeb; border:2px solid #fde68a; border-radius:16px; padding:18px; text-align:center;">
            <div style="font-size:11px; color:#64748b; font-weight:700; text-transform:uppercase; letter-spacing:1px;">💰 Est. Net Profit</div>
            <div style="font-size:26px; font-weight:800; color:#0f172a; margin:8px 0 4px;">₹{_prof_h:,}K</div>
            <div style="font-size:12px; color:#92400e; font-weight:600;">for {y_area:.0f} hectare(s)</div>
        </div>
        <div style="background:#f0f9ff; border:2px solid #bae6fd; border-radius:16px; padding:18px; text-align:center;">
            <div style="font-size:11px; color:#64748b; font-weight:700; text-transform:uppercase; letter-spacing:1px;">📈 Avg Mandi Price</div>
            <div style="font-size:26px; font-weight:800; color:#0f172a; margin:8px 0 4px;">₹{_price_h:,}</div>
            <div style="font-size:12px; color:#0369a1; font-weight:600;">per quintal</div>
        </div>
        <div style="background:#fdf4ff; border:2px solid #e9d5ff; border-radius:16px; padding:18px; text-align:center;">
            <div style="font-size:11px; color:#64748b; font-weight:700; text-transform:uppercase; letter-spacing:1px;">🗓️ Best Time to Sell</div>
            <div style="font-size:20px; font-weight:800; color:{_sell_color}; margin:8px 0 4px;">{_sell_label}</div>
            <div style="font-size:12px; color:#7c3aed; font-weight:600;">Peak month: {_peak_home}</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    # ─────────────────────────────────────────────────────────────────────────
